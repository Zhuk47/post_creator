...
add_action('admin_init', 'RemoveAddMediaButton');

function removeButtonsFromTinyMCE($toolbars) {
    $disableButtons = array('bullist', 'numlist', 'blockquote', 'alignleft', 'alignright', 'aligncenter', 'wp_more', 'wp_adv');
    $temporary = explode(',', $toolbars['toolbar1']);
    foreach ($disableButtons as $disableButton) {
        if (($key = array_search($disableButton, $temporary)) !== false) {
            unset($temporary[$key]);
        }
    }
    $toolbars['toolbar1'] = implode(',', array_values($temporary));
    return $toolbars;
}

add_filter('tiny_mce_before_init', 'removeButtonsFromTinyMCE');
...
//    wp_enqueue_style('nicEditGif', plugins_url('zh_post_editor/assets/nicEdit/nicEditorIcons.gif'));
    wp_enqueue_style('zhstyle', plugins_url('zh_post_editor/assets/nicEdit/zhstyle.css'));
    wp_enqueue_style('fawesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css');
...
//            var html = `<div class="zbox" id="zbox`+row+`"><input id="image_title" name="image_title`+row+`" placeholder="Заголовок картинки"/> <input type="button" id="up" value="&#xf106" class="fa fa-angle-up"> <input type="button" id="down" value="&#xf107" class="fa fa-angle-down"> <div></br></div><img class="imageboxsrc" id="imageboxsrc`+row+`" src=""/><div><input type="hidden" class="button imagebox_image" name="imagebox_image`+row+`" value=""/><div id="zhbut"><button type="button" id="insert-media-button`+row+`" class="upload_image_button button insert-media add_media">Вставить из библиотеки</button> <button type="button" id="zhremove`+row+`" class="remove_image_button button add_media">Удалить изображение</button></div></div><div></br></div><input id="image_src" name="image_src`+row+`" placeholder="Ссылка"/></div>`;
...
function zh_post_editor_print_imagebox_box($post)
{
    ...
    $html = '';
    if ($image_id) {
        $html .= '<div class="zbox" id="zbox0"><input id="image_title" name="image_title0" placeholder="Заголовок картинки"/> <input type="button" id="up" value="&#xf106" class="fa fa-angle-up"> <input type="button" id="down" value="&#xf107" class="fa fa-angle-down">';
        $html .= '<div></br></div>';
        $html .= '<img class="imageboxsrc" id="imageboxsrc0" src="' . $image[0] . '"/>';
        $html .= '<div><input type="hidden" class="button imagebox_image" name="imagebox_image0" value="' . $image_id . '"/><div id="zhbut"><button type="button" id="insert-media-button0" class="upload_image_button button insert-media add_media">Вставить из библиотеки</button> <button type="button" id="zhremove0" class="remove_image_button button add_media">Удалить изображение</button></div></div>';
        $html .= '<div></br></div>';
